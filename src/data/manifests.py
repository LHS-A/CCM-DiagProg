...
def regression_manifest(dataset_dir: Path, task: dict[str, Any], delta_direction: str) -> pd.DataFrame:
    ...
    if task["id"] == "task3":
        ...
        # The paper predicts absolute one-month outcomes from baseline inputs;
        # follow-up measurements are targets only and never semantic inputs.
        for target, followup in [
            ("CFS_1m", "CFS_1m"),
            ("TBUT_1m", "BUT_1m"),
            ("SIT_1m", "SIT_1m"),
            ("OSDI_1m", "OSDI_1m"),
        ]:
            result[target] = _numeric(frame[followup])
