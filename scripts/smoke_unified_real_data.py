...
from train import collect_statistics,prediction_loss,to_device
...
def main():
    ...
    ds=CCMManifestDataset(frame,task,tokenizer,384,max_length=128,clinical_missingness=.5,excluded_clinical_fields={'CFS','TBUT','SIT','OSDI'},partition='train')
    loader=DataLoader(ds,batch_size=2,shuffle=False,num_workers=0);entry={'identity':2,'indices':[1,2,3,4],'task':task,'loaders':{'train':loader}}
    ...
    gap,descriptors,target,nuisance,_=collect_statistics(model,entry,device);prior,_=build_relation_prior(gap,nuisance.numpy(),seed=7,k=2,initial=2,increment=1,maximum=2,confidence=.95);model.set_prior(2,prior.to(device))
    ...
    model.zero_grad(set_to_none=True);out=model(batch['image'],batch['input_ids'],batch['attention_mask'],2,batch['clinical_available'],2);prediction_loss(out['prediction'],batch['target'][:,1:],batch['target_mask'][:,1:],'regression').backward()
